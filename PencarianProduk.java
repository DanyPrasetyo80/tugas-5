...
class Produk {
    ...
    // Menampilkan informasi produk dengan format tabel
    @Override
...
public class PencarianProduk {
    public static void main(String[] args) {
        // Array berisi daftar data produk
        Produk[] daftarProduk = {
        ...
        String kategoriCari = scanner.nextLine(); // Input kategori dari pengguna

        // Input batasan harga dari pengguna
        ...
        // Memanggil fungsi pencarian berdasarkan kriteria
        ...
        // Jika produk ditemukan, tampilkan masing-masing
    ...
    public static ArrayList<Produk> cariProdukByKriteria(Produk[] daftarProduk,
                                                         ...
                                                         double hargaMin,
                                                         double hargaMax) {
        ArrayList<Produk> hasilPencarian = new ArrayList<>(); // Menyimpan produk yang cocok

        for (int i = 0; i < daftarProduk.length; i++) {
            Produk produk = daftarProduk[i];

            // Cek apakah produk sesuai dengan semua kriteria
            if (produk.kategori.equalsIgnoreCase(kategori) &&  // Kategori cocok (tidak sensitif huruf besar/kecil)
                produk.harga >= hargaMin &&                    // Harga lebih besar dari atau sama dengan minimum
                produk.harga <= hargaMax) {                    // Harga lebih kecil dari atau sama dengan maksimum
                hasilPencarian.add(produk); // Tambahkan ke hasil jika cocok
            }
        }
